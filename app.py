...
def insert_data(userid, hivenumber, data):
    ...
    if conn:
            ...
            print(f"{userid}, {hivenumber}, {temperature}, {humidity}, {load}, {sound}, {timestamp}")
            
            cur = conn.cursor()
            cur.execute("""
                INSERT INTO HiveSensorData (userid, hivenumber, temperature, humidity, load, sound, timestamp) 
                VALUES (%s, %s, %s, %s, %s, %s, %s);
            """, (userid, hivenumber, temperature, humidity, load, sound, timestamp))
            conn.commit()
            cur.close()
            conn.close()
            print(f"✅ Stored: {userid} | {hivenumber} | {temperature} | {humidity} | {load} | {sound} | {timestamp}")
        # except Exception as e:
